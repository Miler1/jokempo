const mlpService = require('../services/mlp.service');
const Matrix = require('../models/utils/Matrix');

/**
 * Controller action to handles request and manages action instruction structure
 */
exports.list = async function (req, res) {
    let resp = await mlpService.list();
    ...
    if (resp.error) {
        ...
        // create MLP
        console.log(respData.mlp_config[0].input)
        let hidden = respData.mlp_config[0].hidden;
        let input = respData.mlp_config[0].input;
        let output = respData.mlp_config[0].output;
        let learningRate = respData.mlp_config[0].learningRate;
        let iterations = respData.mlp_config[0].iterations;
